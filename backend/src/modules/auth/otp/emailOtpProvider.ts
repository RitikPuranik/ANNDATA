...
import { passwordResetOtpEmailTemplate } from "../../notifications/email/email.templates";
...
export class EmailOtpProvider implements OtpProvider {
  ...
  async sendOtp(destination: string, purpose: string, userId?: string): Promise<SendOtpResult> {
    ...
    if (!result.success) {
      ...
      logger.error({ destination, purpose, error: result.error }, "[EmailOtpProvider] Failed to send OTP");
  ...
  async verifyOtp(challengeId: string, code: string, expectedPurpose?: string): Promise<VerifyOtpResult> {
    ...
    await this.prisma.otpChallenge.update({
      ...
      data: { attempts: { increment: 1 }, ...(matches ? { consumedAt: new Date() } : {}) },
    });
