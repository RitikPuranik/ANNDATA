...
function Content(){
 ...
 <Card><div className="mb-4 flex items-center gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-accent/20"><BarChart3 className="h-5 w-5"/></span><div><h2 className="section-title">Latest crop-wide forecast</h2><p className="text-sm text-muted-foreground">The backend returns insufficient-data outcomes honestly instead of fabricating a prediction.</p></div></div>{latest.isLoading?<LoadingBlock/>:latest.isError?<p className="text-sm text-muted-foreground">No valid forecast exists yet. Generate one above.</p>:<pre className="max-h-[480px] overflow-auto rounded-2xl bg-[#242424] p-5 text-xs leading-5 text-[#f8f4e9]">{JSON.stringify(latest.data,null,2)}</pre>}</Card></div>
}
